...
export const updateUserData = async(req,res) =>{
    try {
       ...
       if(tempUser.username !== username) {
         ...
         if(user){
            // we will not change username if already taken
...
// find user using username, location , name,email
...
export const followUser = async(req,res) =>{
    try {
        ...
        if(user.following.includes(id)){
            return res.json({success: false, message: "Your already following this user"});
        }
...
export const unfollowUser = async(req,res) =>{
    try {
        ...
        return res.json({success: true, message: "You're no longer following this user"});
    } catch (error) {
        console.log(error.message);
        return res.json({success: false, message:error.message});
    }
...
export const sendConnectionRequest = async(req,res)=>{
    try {
        ...
        // Check user has sent more than 20 connection requests
...
export const acceptConnectionRequest = async(req,res)=>{
    try {
        ...
        const connections = await Connection.findOne({from_user_id: id, to_user_id: userId});
...
export const getUserProfiles = async(req , res )=>{
    try {
        ...
        if (!profile){
             return res.json({success: false, message:"Profile not foud"});
        }
