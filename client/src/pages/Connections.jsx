...
import {Users, UserPlus, UserCheck, UserRoundPen, MessageSquare, MessageSquareHeart} from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { dummyConnectionsData, dummyPendingConnectionsData, dummyFollowersData, dummyFollowingData } from '../assets/assets'
    ...
    <div className='min-h-screen bg-sky-50 '>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab)=>(
              <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={`flex items-center text-sm px-3 py-1 rounded-md transition-colors   cursor-pointer${currentTab === tab.label ?'bg-white font-medium text-black': 'text-gray-500 hover:text-black'}` }>
